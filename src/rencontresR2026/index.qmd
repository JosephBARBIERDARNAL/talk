...
%%| echo: false
flowchart LR
    subgraph interpreted["Langage interprété (R, Python)"]
        direction TB
        source_i["Code source"] --> interpreter["Interpréteur"]
        interpreter --> execution_i["Exécution"]
    end
```
...
%%| echo: false
flowchart LR
    subgraph compiled["Langage compilé (Rust, C++)"]
        direction TB
        source_c["Code source"] --> compiler["Compilateur"]
        compiler --> binary["Programme natif (.exe)"]
        binary --> execution_c["Exécution"]
    end
```
