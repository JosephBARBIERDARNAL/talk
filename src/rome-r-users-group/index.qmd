...
*"I think it's important to compare units sold and sales in the same chart, let's do a barplot instead!"*
...
- <span style="font-size: 1.4em; color: red;">No! Design first, code second</span>
:::

## Let's remove all code we written
...
change_date <- dates[price_change_idx]
df_filter <- df |> filter(date > "2025-07-01")

summary_df <- df |>
    mutate(period = if_else(date < change_date, "Before", "After")) |>
    group_by(period) |>
    summarise(
        avg_units = mean(unit_sold),
...
impact <- summary_df |>
    summarise(
        units_pct = avg_units[period == "After"] /
            avg_units[period == "Before"] -
            1,
        sales_pct = avg_sales[period == "After"] /
            avg_sales[period == "Before"] -
            1,
        units_abs = avg_units[period == "After"] -
            avg_units[period == "Before"],
        sales_abs = avg_sales[period == "After"] - avg_sales[period == "Before"]
    )

p_units <- ggplot(df_filter, aes(date, unit_sold)) +
    geom_line(linewidth = 0.7) +
    geom_vline(xintercept = change_date, linetype = "dashed") +
    annotate(
        "label",
        x = change_date,
        y = max(df$unit_sold),
    ...
    geom_smooth(
        data = ~ filter(.x, date < change_date),
        method = "lm",
        se = FALSE,
        linewidth = 0.8
    ) +
    ...
    labs(
        subtitle = paste0(
            "Avg before: ",
            comma(round(summary_df$avg_units[summary_df$period == "Before"])),
            " | Avg after: ",
            comma(round(summary_df$avg_units[summary_df$period == "After"])),
            " | Change: ",
            comma(round(impact$units_abs, 1)),
            " (",
            percent(impact$units_pct, accuracy = 0.1),
            ")"
        ),
        x = NULL,
        y = "Units sold"
...
p_sales <- ggplot(df_filter, aes(date, sales)) +
    geom_line(linewidth = 0.7) +
    geom_vline(xintercept = change_date, linetype = "dashed") +
    geom_smooth(
        data = ~ filter(.x, date < change_date),
    ...
    labs(
        subtitle = paste0(
            "Avg before: ",
            dollar(round(
                summary_df$avg_sales[summary_df$period == "Before"],
                -2
            )),
            " | Avg after: ",
            dollar(round(
                summary_df$avg_sales[summary_df$period == "After"],
                -2
            )),
            " | Change: ",
            dollar(round(impact$sales_abs, -2)),
            " (+",
            percent(impact$sales_pct, accuracy = 0.1),
            ")"
        ),
        x = NULL,
        y = "Sales"
